...
export default function RealPackageMap({
  ...
  const defaultLat = 0
  const defaultLng = 0
  const hasLocation = packageData.current_location?.latitude != null && packageData.current_location?.longitude != null
  ...
  useEffect(() => {
    ...
    if (mapRef.current && !leafletMapRef.current) {
      const lat = hasLocation ? packageData.current_location.latitude : defaultLat
      const lng = hasLocation ? packageData.current_location.longitude : defaultLng
    ...
    if (leafletMapRef.current) {
      ...
      // Only add current location marker if we have a valid location
      if (hasLocation) {
