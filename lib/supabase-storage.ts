...
export async function initStorage() {
  try {
    ...
    if (getBucketError) {
      ...
      const bucketNotFound = getBucketError.message.toLowerCase().includes("does not exist") ||
                            getBucketError.message.toLowerCase().includes("not found") ||
                            getBucketError.message.toLowerCase().includes("bucket not found")
      
      if (bucketNotFound) {
        // Create the bucket if it doesn't exist
        console.log("Bucket not found, creating package-images bucket...")
        ...
        // Don't throw - just return error to prevent app crash
        return { success: false, error: `Failed to check bucket: ${getBucketError.message}` }
      }
    } else {
