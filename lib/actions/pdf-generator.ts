...
export async function generatePDF(packageData: PackageData): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const qrCodeDataUrl = await QRCode.toDataURL(`https://globetrack.vercel.app/tracking/${packageData.tracking_number}`, {
      ...
      doc.font('Courier-Bold')
         .fontSize(11)
         .fillColor('white')
         .text('Globetrack services', 40, footerTop + 15);
