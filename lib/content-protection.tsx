...
export function useProtectedContent(encryptedContent: string, encryptionKey = "globetrack") {
  const [content, setContent] = useState<string>("")
...
export function saveContentSections(sections: ContentSection[]) {
  if (typeof window !== "undefined") {
    localStorage.setItem("globetrack_content_sections", JSON.stringify(sections))
  }
}

export function getContentSections(): ContentSection[] {
  if (typeof window !== "undefined") {
    const stored = localStorage.getItem("globetrack_content_sections")
    if (stored) {
      try {
